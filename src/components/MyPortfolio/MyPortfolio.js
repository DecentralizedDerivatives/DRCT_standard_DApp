...
class MyPortfolio extends Component {
  ...
  getmyTransactions = async () => {
    ...
    var ats = [{
          ...
          ContractCreation: await factory.ContractCreation({}, {fromBlock:0, toBlock: 'latest'})
          //Transfer: await drct.Transfer({}, {fromBlock:0, toBlock: 'latest'}),
          //Approval: await drct.Approval({}, {fromBlock:0, toBlock: 'latest'})
    }]

    for(let i = 0; i < titles.length;i++){
      let transferEvent = await factory.ContractCreation({}, {fromBlock:0, toBlock: 'latest'});
      await transferEvent.get((error, logs) => {
            for(let j = logs.length-1; j >= Math.max(logs.length-10,0); j--){
             if(logs[i].args["_sender"].toUpperCase() == this.state.myAccount.toUpperCase()){
